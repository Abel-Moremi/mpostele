...
This repository is a local-first pipeline for generating autonomous marketing videos and posters using an LLM agent swarm plus code-driven rendering (Remotion, via headless Chromium) — no diffusion model, no GPU dependency. The core principle is the **Sequential Execution Contract**: every stage, LLM call or render, runs as a transient, single-responsibility subprocess.
...
- Run the LLM planning layer (`Qwen2.5-1.5B` via Ollama) and the Remotion render stage without either being resident at the same time
- Keep rendering deterministic: a small, hand-written library of scene/poster components in `remotion/src/`, driven only by agent-generated *data* (which components, what text, what timing) — never agent-generated JSX
- Keep the workflow reproducible and debuggable via a single `state.json` per job

## Non-negotiable constraints

- Every generation step (LLM call, Remotion render, encode) runs in its own subprocess via `subprocess.run` — never imported into one long-lived process
...
2. Video and poster rendering via the `remotion/` Node project (`npx remotion render` / `npx remotion still`), never a diffusion model
...
- pipeline branches by `media_type`: poster (`poster_layout_agent` → `poster_validator` → `poster_engine`'s `remotion still`) or video (`composition_agent` → `composition_validator` → `video_engine`'s `remotion render` → `encode`)
...
- Never let an agent generate render code (JSX/Python compositing logic) — only data that flows into hand-written, reused components

## Project-specific guardrails

- Do not add a persistent worker daemon, resident model server, or long-lived render process as a default path
- Do not reintroduce a diffusion model (local or remote) without a clear reason the Remotion path can't cover — it was removed after being confirmed less reliable (driver/VRAM/network dependent) than code-driven rendering
...
- the Remotion scene/poster component library and its data contract with the Python agents
