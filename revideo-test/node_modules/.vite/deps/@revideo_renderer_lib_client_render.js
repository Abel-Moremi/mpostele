import { I as Renderer, V as getFullRenderingSettings, et as Color, ot as Vector2 } from "./lib-lNhSsmSc.js";
//#region node_modules/@revideo/renderer/lib/client/render.js
/**
* Render the project.
*/
var render = async (project, workerId, totalNumOfWorkers, hiddenFolderId, overwriteRenderSettings) => {
	try {
		...
		if (overwriteRenderSettings.background) background = new Color(overwriteRenderSettings.background);
		let size = renderSettingsFromProject.size;
		if (overwriteRenderSettings.size) size = new Vector2(overwriteRenderSettings.size.x, overwriteRenderSettings.size.y);
		const combinedSettings = {
			...
			background,
			size,
			range: [renderer.frameToTime(firstWorkerFrame), renderer.frameToTime(lastWorkerFrame)]
		};
...
async function getGlobalFirstAndLastFrame(project, renderer, startSecondFromUser, endSecondFromUser) {
	...
	let lastGlobalFrame;
	if (isFinite(endSecondFromUser)) lastGlobalFrame = renderer.timeToFrame(endSecondFromUser);
	else {
		const settings = {
...
async function getWorkerFirstAndLastFrame(firstGlobalFrame, lastGlobalFrame, workerId, totalNumOfWorkers) {
	...
	return {
		firstWorkerFrame,
		lastWorkerFrame: Math.min(firstWorkerFrame + framesPerWorker - offset, lastGlobalFrame)
	};
}
//#endregion
export { render };
