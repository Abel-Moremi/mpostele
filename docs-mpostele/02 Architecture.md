...
The frontend Plan workspace is the human approval boundary. A three-step setup emphasizes schedule, creative prompt, and generation; examples make the prompt easier to author, technical options and existing-plan loading stay secondary, and the review summary exposes duration, word count, and decision progress. Its loopback-only server adapter invokes the planner without a shell, limits paths to the repository, validates the prompt, and limits schedule dates to a rolling seven-day horizon (today through six days ahead). Edits reset affected scenes and the plan to pending review. An approved plan can be converted to a `status: draft` render-job manifest containing only approved scenes. Screenshot evidence becomes an image source, narration becomes a script, and platform choice determines the export preset. Conversion writes JSON and can load the draft into Render, but rendering remains a separate explicit action. Plan evidence thumbnails and completed-video previews are served by a loopback-only media route that enforces repository containment and a small media-type allowlist; video byte ranges use native browser controls without adding an editor dependency.
