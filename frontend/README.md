...
- a Plan workspace with a guided creative prompt, rolling seven-day schedule, local plan generation/loading, scene editing and ordering, evidence-reference review, approval, and draft render-manifest conversion
...
The **Plan** workspace uses the current local date to expose a rolling seven-day window (today through six days ahead). Choose a day, write a creative-direction prompt, provide a completed discovery `snapshot.json` and optional `review.json`, and generate with the deterministic provider or a loopback-only llama.cpp endpoint. The prompt is required, limited to 2,000 characters, saved in the plan brief, and never overrides the evidence boundary. The deterministic provider uses prompt and objective keywords to prioritize matching states; llama.cpp can additionally shape scene selection and wording. Prompt examples clarify the expected input, while advanced settings and existing-plan loading remain progressively disclosed. Plan paths, prompts, and scheduling settings persist in browser SQLite; generated plans remain ordinary inspectable JSON files in the repository.

Each scene displays a lightweight screenshot thumbnail, its evidence state, and its screenshot path. The review summary reports active duration, narration word count, and pending/approved/rejected counts so pacing and review progress are visible before approval. A saved/unsaved indicator separates local edits from approved review state. Editing purpose, narration, duration, overlay text, or ordering returns the plan to `pending_review`. Scenes can be approved or rejected individually; **Approve plan** approves every non-rejected scene. Conversion is deliberately blocked until the plan is approved and creates a `status: "draft"` render manifest from approved scenes only. **Create draft and open in Render** loads that manifest into the Render editor but still requires an explicit render action. The conversion uses the captured screenshots as image sources, narration as local TTS scripts, and platform-appropriate export presets. It does not render or publish automatically.

The loopback-only `/api/content-plans` endpoint enforces repository path containment for every operation and validates schema/version, schedule horizon, the required prompt and its length, scene review states, numeric limits, and the local model endpoint before generation, saving, or conversion.
