...
- live discovery progress, stop control, viewport presets, a state/transition graph, screenshot and website-structure evidence browsing, review decisions, important-flow markers, and targeted follow-up runs
- a prompt-first Plan workspace that develops seven editable daily content directions, supports evidence-reference review, and converts a selected approved day into a draft render manifest
...
After completion, **Use this discovery in Plan** passes the generated snapshot path directly to Plan. The frontend then shows coverage metrics and a bounded state/transition graph. Selecting a graph node or evidence link opens the captured screenshot beside the detected heading hierarchy, semantic landmarks, navigation groups, forms, visible text, and controls. Pages and transitions can be marked important, while ambiguous actions can be approved or rejected for planning. These annotations are validated against the run and stored locally in `review.json`; approval does not bypass the agent's conservative click policy. **Explore from this page** starts a new isolated discovery run at that page for targeted follow-up.

The complete reusable data remains in `snapshot.json` and `knowledge.sqlite`; a human-readable page and navigation inventory is also written to `reports/site-structure.md`. Structural snapshot data uses the additive discovery schema 1.1.0 field `observation.structure`. The UI bounds graph and inventory data to keep browser memory modest, and the evidence endpoint only serves run-contained PNG files. Discovery settings persist locally, while generated evidence and review annotations remain in the configured project-contained artifact folder.
...
The **Plan** workspace starts with a required campaign prompt rather than a single publishing day. After describing the desired outcome, choose a start date from the rolling seven-day window, provide a completed discovery `snapshot.json` and optional `review.json`, and generate with the deterministic provider or a loopback-only llama.cpp endpoint. The prompt is limited to 2,000 characters, saved in the plan brief, and never overrides the evidence boundary. The deterministic provider uses prompt and objective keywords to prioritize matching states; llama.cpp can additionally shape scene selection and wording. Plan paths, prompts, and scheduling settings persist in browser SQLite; generated plans remain ordinary inspectable JSON files in the repository.

Generation creates seven consecutive daily post cards. Each card has an editable theme, content direction, hook, format, review state, and references to one or more reusable evidence scenes. Each scene displays a lightweight screenshot thumbnail, its evidence state, and its screenshot path. Editing a daily direction or scene returns the affected item and plan to `pending_review`; **Approve plan** approves all non-rejected days and scenes. Conversion is blocked until approval. Select an approved daily card to create a `status: "draft"` render manifest containing only that day's approved scene references. **Open selected day in Render** loads the draft into the Render editor but still requires an explicit render action. It does not render or publish automatically.

The loopback-only `/api/content-plans` endpoint enforces repository path containment for every operation and validates content-plan schema 1.1, campaign start horizon, seven consecutive daily directions, scene references, the required prompt and its length, review states, numeric limits, and the local model endpoint before generation, loading, saving, or conversion.
